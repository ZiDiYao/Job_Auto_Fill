...
- Uses White + Green by default, with Blue available as an optional appearance theme; the selected theme is saved immediately and shared by the popup and settings pages.
...
- Includes bundled platform adapters for Workday, Dayforce, Indeed, LinkedIn, Greenhouse, Lever, SmartRecruiters, iCIMS, Oracle Taleo, SAP SuccessFactors, Oracle Recruiting, Ashby, ADP, UKG/UltiPro, BambooHR, Jobvite, JazzHR, Recruitee, Pinpoint, Workable, Teamtailor, Personio, Breezy HR, Comeet, Rippling, Paylocity, Cornerstone, Avature, Eightfold, Phenom, and Zoho Recruit. Other company career sites fall back to the same generic semantic-DOM engine, including open Shadow DOM controls and application forms embedded in same-origin or permitted frames.
- Keeps all selectors, dropdown interactions, waits, date entry, navigation, and verification logic inside the published extension package. AI responses are limited to schema-validated classifications and suggested values; providers cannot send CSS selectors, JavaScript, or browser actions.
