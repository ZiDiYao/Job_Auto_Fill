...
- Includes a browser-only Skills Preview page that shows the neutral ATS tags and ranking reasons produced by the current limits without calling an AI provider; a user-managed permanent blacklist is enforced by both backend ranking and final browser insertion.
